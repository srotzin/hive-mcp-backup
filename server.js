...
const SUBSCRIPTION_TIERS = {
  starter:    { price_usd: 25.0, calls_per_day: 100, label: 'Starter' },
  pro:        { price_usd: 99.0, calls_per_day: 1000, label: 'Pro' },
...
async function emitSpectralReceipt({ event_type, did, amount_usd, tool_name, tx_hash, metadata }) {
  ...
  try {
    ...
    console.warn('[backup] receipt emit failed (non-fatal):', _.message || _);
  }
}
...
app.post('/v1/subscription', async (req, res) => {
  ...
  if (tier !== 'enterprise' && !tx_hash) {
    return res.status(402).json({
      error: 'payment_required',
      x402: {
        type: 'x402', version: '1', kind: 'subscription_backup',
  ...
  await emitSpectralReceipt({
    ...
    tx_hash: tx_hash || null,
    metadata: { tier, service: 'Hive Backup', expires_ms: record.expires_ms },
  });
  ...
  return res.json({
    ...
    partner_attribution: 'Agent backup-as-a-service — Spectral-signed disaster recovery attestation. Hive attests; storage on S3/GCS/Arweave.',
    brand: '#C08D23',
